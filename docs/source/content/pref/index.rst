...
После старта игры, кроме основного сообщения от сервера (о нём ниже), так же прилетит событие "game.player.start" (каждому игроку - своё сообщение):

.. literalinclude:: code/game_start.json

Список карт будет не "null" - в трёх случаях: если это предыдущий игрок, если это текущий игрок или если начался новый раунд игры.
Список доступных действий будет не "null" только у текущего игрока.
Флаг-предупреждение о том, что следующий пропуск для игрока будет последним, будет положительным только у предыдущего игрока после второго пропуска подряд.
...
.. literalinclude:: code/move_prev/offer_approve.json
...
**Игрок заявляет о взятках**

Во время игры игрок может предложить другим игрокам закончить раздачу и согласиться с тем, что контрактник взял N-взяток.

Заявить о взятках
------------------------------------

Если игрок может заявить о взятках, он может отправить запрос на "game.offer" с параметром

.. literalinclude:: code/offer/offer_request.json

После того, как игрок сделает предложение - другим игрокам (двум - если это мизер, и одному вистующему в светлую  - если ставка) придут сообщения "player.game.move":

.. literalinclude:: code/offer/offer.json

Если у предлагающего игрока не были видны карты - то из этого сообщения как раз нужно взять и временно показать.

Отвергнуть предложение
------------------------------------

Если игрок отвергает предложение, он должен отправить запрос на "game.offer_reject".
После этого всем игрокам придёт сообщение "player.game.move":

.. literalinclude:: code/offer/offer_reject.json

Согласиться с предложением
------------------------------------

Если игрок согласен с предложением, он должен отправить запрос на "game.offer_approve".
Если все игроки, которым было отправлено сообщение - согласились, то произойдёт обычное завершение раунда и придёт информация о следующем ходе.
...
Если из игры выйдет все три игрока, то игра завершится.
Если игрок "пропустит" свой ход, но всё ещё будет сидеть за столом - два хода за него сделает сервер, но на третий раз - его выкинет из игры и за него сядет бот.